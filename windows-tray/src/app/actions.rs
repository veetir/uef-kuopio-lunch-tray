...
impl App {
    ...
    /// Changes how menu item rows are rendered in the popup.
    pub fn set_lunch_item_display_mode(&self, mode: crate::settings::LunchItemDisplayMode) {
        let mut state = self.state.lock().unwrap();
        state.settings.lunch_item_display_mode = mode;
        let _ = save_settings(&state.settings);
    }
